...
describe("PII Detection", () => {
  describe("detectPII", () => {
    ...
    it("should detect phone numbers", () => {
      ...
      const phoneSpans = result.spans.filter(
        (span) => span.type === PIIType.Phone,
      );
    ...
    it("should detect credit card numbers", () => {
      ...
      const cardSpans = result.spans.filter(
        (span) => span.type === PIIType.CreditCard,
      );
    ...
    it("should detect IP addresses", () => {
      ...
      const ipSpans = result.spans.filter(
        (span) => span.type === PIIType.IPAddress,
      );
    ...
    it("should detect multiple PII types in single text", () => {
      const text =
        "Contact John Doe at john@example.com or call (555) 123-4567. SSN: 555-55-5555";
      const result = detectPII(text);
    ...
    it("should provide confidence levels", () => {
      ...
      expect(Object.values(ConfidenceLevel)).toContain(
        result.spans[0].confidence,
      );
    });
    ...
    it("should handle confidence threshold filtering", () => {
      ...
      expect(lowThresholdResult.spans.length).toBeGreaterThanOrEqual(
        highThresholdResult.spans.length,
      );
    });
    ...
    it("should detect names with NLP when enabled", () => {
      ...
      if (result.hasPII) {
        const nameSpans = result.spans.filter(
          (span) => span.type === PIIType.Name,
        );
    ...
    it("should handle context extraction", () => {
      const text =
        "The user email address is user@example.com and should be verified.";
      const result = detectPII(text, {
        includeContext: true,
        contextWindow: 5,
      });
    ...
    it("should handle strict mode", () => {
      ...
      expect(strictResult.spans.length).toBeLessThanOrEqual(
    ...
    it("should provide suggestions for detected PII", () => {
      ...
      expect(result.suggestions.some((s) => s.includes("Critical PII"))).toBe(
        true,
      );
    });
  });
  ...
  describe("countPIIByType", () => {
    it("should count occurrences by PII type", () => {
      const text =
        "Emails: john@example.com, jane@test.com. Phone: 555-123-4567";
  ...
  describe("Pattern Management", () => {
    it("should get default patterns", () => {
      ...
      expect(patterns.every((p) => p.type && p.regex && p.description)).toBe(
        true,
      );
    });
