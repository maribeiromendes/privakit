...
describe('GDPR Compliance Test Suite', () => {
  ...
  describe('Article 6 - Lawful basis for processing', () => {
    it('should require explicit consent for sensitive data processing', () => {
      ...
      const ssnDecision = gdprEngine.evaluate(PIIType.SSN, PolicyOperation.Store);
      expect(ssnDecision.requiresEncryption).toBe(true);
    });

    it('should allow legitimate interest for basic contact processing', () => {
      const contactDecision = gdprEngine.evaluate(PIIType.Email, PolicyOperation.Display);
      expect(contactDecision.allowed).toBe(true);
      
      const phoneDecision = gdprEngine.evaluate(PIIType.Phone, PolicyOperation.Display);
    ...
    it('should restrict processing without lawful basis', () => {
      ...
      const transferDecision = gdprEngine.evaluate(PIIType.Name, PolicyOperation.Transfer);
  ...
  describe('Article 15 - Right of access by the data subject', () => {
    it('should support data subject access requests', () => {
      const personalData = 'John Doe, email: john@example.com, phone: (555) 123-4567';
  ...
  describe('Article 17 - Right to erasure (right to be forgotten)', () => {
    it('should support complete data removal', () => {
      const userData = 'Customer: Alice Smith, SSN: 555-12-3456, Email: alice@test.com';
  ...
  describe('Article 20 - Right to data portability', () => {
    it('should extract data in structured format', () => {
      const customerData = 'Profile: Jane Doe, jane@company.com, +44-20-7946-0958, London UK';
  ...
  describe('Article 25 - Data protection by design and default', () => {
    it('should apply privacy-preserving defaults', () => {
      const personalData = 'Employee: Bob Wilson, ID: 123456789, Salary: $75000';
    ...
    it('should minimize data exposure through progressive masking', () => {
      const fullRecord = 'Patient: Mary Johnson, DOB: 1985-03-15, SSN: 987-65-4321, Address: 456 Oak Street';
      
      // Test that different data types can be masked appropriately
      const nameResult = maskPII('Mary Johnson', PIIType.Name, { preserveStart: 2, preserveEnd: 2 });
  ...
  describe('Chapter V - Transfers of personal data to third countries', () => {
    it('should restrict cross-border data transfers', () => {
      const transferDecision = gdprEngine.evaluate(PIIType.Email, PolicyOperation.Transfer);
    ...
    it('should require encryption for international processing', () => {
      const processDecision = gdprEngine.evaluate(PIIType.CreditCard, PolicyOperation.Process);
  ...
  describe('Data minimization principle', () => {
    it('should process only necessary data fields', () => {
      const excessiveData = 'Order: #12345, Customer: John Smith, Email: john@test.com, Phone: 555-0123, SSN: 123-45-6789, Mother maiden name: Johnson';
  ...
  describe('Purpose limitation principle', () => {
    it('should restrict data use beyond original purpose', () => {
      ...
      const marketingDecision = gdprEngine.evaluate(PIIType.Email, PolicyOperation.Log);
      expect(marketingDecision.allowed).toBe(false);
      
      // Analytics should require anonymization
      const analyticsDecision = gdprEngine.evaluate(PIIType.Name, PolicyOperation.Process);
  ...
  describe('Storage limitation principle', () => {
    it('should enforce data retention policies', () => {
      const emailDecision = gdprEngine.evaluate(PIIType.Email, PolicyOperation.Store);
  ...
  describe('Accountability principle', () => {
    it('should provide audit trail for compliance demonstration', () => {
      const auditText = 'Processing customer data: name=John Doe, email=john@example.com';
  ...
  describe('Special categories of personal data (Article 9)', () => {
    it('should apply enhanced protection for sensitive data', () => {
      ...
      const healthDecision = gdprEngine.evaluate(PIIType.DriverLicense, PolicyOperation.Store);
      ...
      const biometricDecision = gdprEngine.evaluate(PIIType.SSN, PolicyOperation.Process);
  ...
  describe('GDPR compliance workflow integration', () => {
    it('should support end-to-end GDPR compliant processing', () => {
      const gdprWorkflow = 'Data subject request: Access all data for user@example.com';
      ...
      const accessDecision = gdprEngine.evaluate(PIIType.Email, PolicyOperation.Display);
