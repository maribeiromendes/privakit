...
const NAME_PATTERNS = {
  ...
  multiWordName:
    /^[a-zA-ZÀ-ÿĀ-žА-я\u0100-\u017F\u0180-\u024F\u1E00-\u1EFF\s\-'.]{2,}$/,

  // Title case pattern
  titleCase:
    /^[A-ZÀ-ÿĀ-žА-я\u0100-\u017F\u0180-\u024F\u1E00-\u1EFF][a-zA-ZÀ-ÿĀ-žА-я\u0100-\u017F\u0180-\u024F\u1E00-\u1EFF\-']*(\s[A-ZÀ-ÿĀ-žА-я\u0100-\u017F\u0180-\u024F\u1E00-\u1EFF][a-zA-ZÀ-ÿĀ-žА-я\u0100-\u017F\u0180-\u024F\u1E00-\u1EFF\-']*)*$/,
  ...
  try {
    ...
    if (people.length > 0) {
      ...
      if (nameWords.length === 1) {
        ...
        // Check for suffix
        if (
          NAME_SUFFIXES.has(nameWords[endIndex].toLowerCase().replace(".", ""))
        ) {
          if (opts.allowSuffix) {
      ...
      if (
        NAME_PATTERNS.multiWordName.test(trimmedName) ||
        NAME_PATTERNS.singleName.test(trimmedName)
      ) {
        isLikelyName = true;
        confidence = ConfidenceLevel.Medium;
        nameType = "person";
