...
const DEFAULT_PII_PATTERNS: PIIPattern[] = [
  ...
  },
  {
    type: PIIType.Phone,
    regex:
      /(?:\+?1[-.\s]?)?\(?([2-9][0-8][0-9])\)?[-.\s]?([0-9]{3})[-.\s]?([0-9]{4})\b/g,
    description: "US phone numbers",
    riskLevel: "high" as any,
    examples: ["+1 (555) 123-4567", "555-123-4567", "5551234567"],
    falsePositiveFilters: [
      (match) =>
        !/^(000|111|222|333|444|666|777|888|999)/.test(
          match.replace(/\D/g, ""),
        ), // Invalid area codes (removed 555 which is used in examples)
      (match) => {
...
export function detectPII(
  ...
  for (const pattern of patterns) {
    ...
    for (const match of matches) {
      ...
      if (pattern.falsePositiveFilters) {
        isValid = pattern.falsePositiveFilters.every((filter) =>
          filter(matchedText),
        );
      }
      ...
      switch (pattern.type) {
        case PIIType.Email:
          const emailValidation = validateEmail(matchedText, {
            allowDisplayName: false,
          });
          additionalValidation = emailValidation.isValid;
          confidence = emailValidation.isValid
        ...
        case PIIType.Phone:
          ...
          additionalValidation =
            ...
            !!phoneValidation.isPossible ||
            matchedText.replace(/\D/g, "").length >= 10; // At least 10 digits
          confidence = phoneValidation.isValid
            ? ConfidenceLevel.High
            : phoneValidation.isPossible
              ? ConfidenceLevel.Medium
              : matchedText.replace(/\D/g, "").length >= 10
                ? ConfidenceLevel.High
                : ConfidenceLevel.Low;
          break;
      ...
      const span: DetectionSpan = {
        ...
        metadata: {
          ...
          context: options.includeContext
            ? extractContext(text, start, end, options.contextWindow || 10)
            : undefined,
        },
      };
  ...
  if (detectedTypes.size > 0) {
    suggestions.push(
      "Consider masking or redacting detected PII before logging or storing",
    );

    if (
      detectedTypes.has(PIIType.SSN) ||
      detectedTypes.has(PIIType.CreditCard)
    ) {
      suggestions.push(
        "Critical PII detected - ensure encryption and secure handling",
      );
    }

    if (detectedTypes.has(PIIType.Email) || detectedTypes.has(PIIType.Phone)) {
      suggestions.push(
        "Contact information detected - verify consent for processing",
      );
    }
  }
...
function detectNamesWithNLP(
  ...
  try {
    ...
    people.forEach((person: any) => {
      ...
      const nameValidation = validateName(personText, {
            ...
            metadata: {
              ...
              context: options.includeContext
                ? extractContext(
                    text,
                    match.index,
                    match.index + personText.length,
                    options.contextWindow || 10,
                  )
                : undefined,
            },
          });
    ...
    if (organizations.length > 0) {
      suggestions.push(
        "Organization names detected - verify if these should be treated as PII",
      );
    }
    ...
    suggestions.push(
      "NLP processing failed - consider manual review for name detection",
    );
  }
...
function calculateOverallConfidence(spans: DetectionSpan[]): ConfidenceLevel {
  ...
  const confidenceValues = spans.map((span) =>
    getConfidenceValue(span.confidence),
  );
  const average =
 ...
 */
export function countPIIByType(
  text: string,
  options: DetectionOptions = {},
