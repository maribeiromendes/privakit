...
function processUserData(email: string, operation: string) {
  ...
  if (!decision.allowed) {
    throw new Error(
      `Operation '${operation}' not permitted: ${decision.reason}`,
    );
  }
...
class ApprovalWorkflowEngine {
  ...
  async processApproval(
    ...
    const approval = {
      ...
      expiresAt:
        decision === "approve"
          ? new Date(Date.now() + this.getApprovalDuration(request))
          : null,
    };
    ...
    this.addRule("minimum_necessary", {
      ...
      evaluate: (context) => {
        ...
        if (!legitimatePurposes.includes(purpose)) {
          return {
            allowed: false,
            reason: "Not a legitimate healthcare purpose",
          };
        }
        ...
        if (!rolePermissions[userRole]?.includes(purpose)) {
          return {
            allowed: false,
            reason: "Role not authorized for this purpose",
          };
        }
